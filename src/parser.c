...
void compileAssignSt(void) {
  ...
  // <code of l-value v>   pushes the address of v (done inside compileLValue)
  // <code of exp>         pushes the value of exp
  // ST                    stores exp's value into v
  Type* varType;
  Type* expType;
...
void compileIfSt(void) {
  ...
  // if-then:      <code dk>; FJ L; <code st>; L:
  // if-then-else: <code dk>; FJ L1; <code st1>; J L2; L1: <code st2>; L2:
  Instruction* fjInst;
  Instruction* jInst;
...
void compileWhileSt(void) {
  ...
  // L1: <code dk>; FJ L2; <code st>; J L1; L2:
  CodeAddress L1;
  Instruction* fjInst;
...
void compileForSt(void) {
  ...
  //   <code lvalue v>; CV; <code exp1>; ST
  //   CV; LI;
  // L2: <code exp2>; LE; FJ L3; <code st>;
  //   CV;CV;LI;LC 1;AD;ST;
  //   CV; LI; J L2
  // L3: DCT 1
  Type* varType;
  ...
  varType = compileLValue();
  genCV();           // duplicate addr(v): one copy is kept on the stack for the whole loop
  ...
  genCV();           // duplicate the persistent addr(v)
  genLI();           // dereference -> current value of v

  L2 = getCurrentCodeAddress();  // loop-back target: re-enter right before exp2
  type = compileExpression();
  checkTypeEquality(varType, type);
  ...
  genCV();           // refresh [addr(v), value(v)] on the stack
  genLI();
  genJ(L2);           // jump back to re-run exp2's comparison

  updateFJ(fjInst, getCurrentCodeAddress());
  genDCT(1);         // drop the persistent addr(v) slot
}
...
void compileCondition(void) {
  // generate code for condition: <code exp1>; <code exp2>; <comparator op>
  Type* type1;
  Type* type2;
  TokenType op;
...
Type* compileExpression(void) {
  // generate code for expression (leading unary + / -)
  Type* type;
...
Type* compileExpression3(Type* argType1) {
  // generate code for expression3: left value already on stack,
  // each +/- term pushes its value then combines with AD/SB
  Type* argType2;
  Type* resultType;
...
Type* compileTerm2(Type* argType1) {
  // generate code for term2: left value already on stack,
  // each * / term pushes its value then combines with ML/DV
  Type* argType2;
  Type* resultType;
...
Type* compileFactor(void) {
  ...
  case TK_IDENT:
	...
	break;
      case TP_CHAR:
	type = charType;
	genLC((int)obj->constAttrs->value->charValue);
	break;
      default:
	break;
      }
